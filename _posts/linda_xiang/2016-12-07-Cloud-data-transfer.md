---
layout: post
title:  "Use Git to Manager the Big Genomic Data Migration in Cloud"
breadcrumb: true
author: linda_xiang
date: 2016-12-03
...
In the age of Big Genomic Data, many academic institutions are faced with the challenge of sharing genomic research data to investigators and research scientists worldwide. The clouds offer scalable and competitively priced computing resources for the analysis and storage of data from large-scale genomics studies. They are now offered by both commercial companies including Amazon, Google and Microsoft, as well as some academic data centres such as Collaboratory in OICR. Provided the cloud infrastructures and computing enviroments are well created, migrating the big genomic data among those cloud storage systems becomes crucial and urgent.

While migrating the tremendous amount of data generated by NGS among those clouds, it is more efficient and realistic to use as many computers as needed to do the data migration parallelly. Given the big size of each file, it is already a lot to handle even using just a single node. When multiple machines are doing the jobs at the same time, things can quickly get complicated and tough. 

Therefore, a real-time management system is needed to synchronize all jobs status which are done by different machines. We had to come up with a simple, yet flexible, efficient and intelligent way to manage thwhole workflow.
...
Here I will give a concrete example to show you how to migrate PCAWG data from data source([PCAWG in GNOS](http://pancancer.info/)) to data destination([Collaboratory](https://www.cancercollaboratory.org/)). Following is the
