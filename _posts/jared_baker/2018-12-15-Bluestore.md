...
### Draining
To drain a node we would modify the osd crush weight of all of a storage nodes OSD's to 0. This will trigger an evacuation of data away from this storage node and distribute it to the rest of the cluster.

Example, to drain OSD's 720 to 755 we would run:
~~~bash
...
[Available in script form here](https://github.com/CancerCollaboratory/infrastructure/blob/master/migrate_bluestore.sh)
...
Notice that when comparing the output of ceph osd tree before/after conversion that the class column may now indicate your osd type (ssd or hdd) which for us, was blank before.
...
Example, to fill OSD's 720 to 755 (12TB disks) we would run:
~~~bash
...
We use a variety of tools to monitor and report on our OpenStack and Ceph cluster. For this migration we were able to monitor filestores xfs data partition and see the data be copied off to the rest of the cluster.
<figure>
...
## Issues
Late in to the project we started to get ceph health warnings, complaining that the monmap was too large. Our cluster is a few years old so our monmap is leveldb. The monmap was growing to to 20GB during the migrations (Under 1GB normally). Leveldb can perform poorly during large data movement events and cause everything to slow down. To remedy this we had to enable mon compaction in our /etc/ceph/ceph.conf and then restart ceph-mon and then wait for the compaction to happen.
~~~bash
[mon]
